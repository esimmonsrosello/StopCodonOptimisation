# v1.5 - - 20250625 No more charts they were gitting in the way 
...
#get it to recognise Us and maybe coding sequences. 
...
def find_start_position(dna_seq):
    global logger
    try:
        dna_seq_upper = dna_seq.upper().replace('U', 'T')
        # Check if sequence starts with ATG
        if dna_seq_upper.startswith('ATG'):
            return 3  # Start after ATG
         
        # Look for ACCATG
        accatg_pos = dna_seq_upper.find('ACCATG')
        if accatg_pos != -1:
            return accatg_pos + 6  # Start after ACCATG
        
        # If neither ATG nor ACCATG found, return None
        return None
    except Exception as e:
        logger.error(f"Error finding start position: {e}", exc_info=True)
        return None
...
def number_of_Slippery_motifs(dna_seq):
    ...
    try:
        dna_seq_upper = dna_seq.upper().replace('U', 'T')
        start_pos = find_start_position(dna_seq_upper)
        if start_pos is None:
            logger.warning("No valid start position found for slippery motif analysis")
            return 0
         
        # Count slippery motifs starting from the determined position, in-frame only
        slippery_count = sum(1 for i in range(start_pos, len(dna_seq_upper) - 3, 3) if dna_seq_upper[i:i+4] in Slippery_Motifs)
        logger.info(f"Slippery motif analysis completed: {slippery_count} motifs found")
...
def number_of_plus1_stops(dna_seq):
    ...
    try:
        ...
        plus1_codons_list = [dna_seq_upper[i:i+3] for i in range(plus1_start, len(dna_seq_upper)-2, 3) if i+3 <= len(dna_seq_upper)]
    ...
    try:
        if choice == 1:
            ...
            data = {
                ...
                'Sequence_Info': [f"Seq len: {len(clean_seq)} bp", f"Prot len: {len(protein_seq)} aa", f"+1 Frame: {plus1_len} codons", f"Stop density: {plus1_stop_counts['total']/max(1,plus1_len):.3f}", f"Slippery density: {slippery_count}"]
            }
